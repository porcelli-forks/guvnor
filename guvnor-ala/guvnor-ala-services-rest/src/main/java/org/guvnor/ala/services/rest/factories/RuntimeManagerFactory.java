...
public class RuntimeManagerFactory {
    ...
    public void startRuntime( RuntimeId runtimeId ) {
        final Optional<RuntimeManager> runtimeManager = managers.stream()
                .filter( p -> p.supports( runtimeId ) )
                .findFirst();
        if ( runtimeManager.isPresent() ) {
             runtimeManager.get().start( runtimeId );
        }
    }
    ...
    public void stopRuntime( RuntimeId runtimeId ) {
        final Optional<RuntimeManager> runtimeManager = managers.stream()
                ...
                .findFirst();
        if ( runtimeManager.isPresent() ) {
             runtimeManager.get().stop( runtimeId );
        }
    }
    ...
    public void restartRuntime( RuntimeId runtimeId ) {
        final Optional<RuntimeManager> runtimeManager = managers.stream()
                ...
                .findFirst();
        if ( runtimeManager.isPresent() ) {
             runtimeManager.get().restart( runtimeId );
        }
    }
    ...
    public void refreshRuntime( RuntimeId runtimeId ) {
        final Optional<RuntimeManager> runtimeManager = managers.stream()
                ...
                .findFirst();
        if ( runtimeManager.isPresent() ) {
             runtimeManager.get().refresh(runtimeId );
        }
    }
