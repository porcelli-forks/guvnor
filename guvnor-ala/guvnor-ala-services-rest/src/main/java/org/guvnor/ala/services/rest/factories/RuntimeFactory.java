...
public class RuntimeFactory {
    ...
    public Optional<Runtime> newRuntime( RuntimeConfig config ) {
        final Optional<RuntimeBuilder> runtimeBuilder = builders.stream()
                .filter( p -> p.supports( config ) )
                .findFirst();
        if ( runtimeBuilder.isPresent() ) {
            return (Optional<Runtime>) runtimeBuilder.get().apply( config );
        }
        return Optional.empty();
    }
    
    public void destroyRuntime( RuntimeId runtimeId ) {
        final Optional<RuntimeDestroyer> runtimeDestroyer = destroyers.stream()
                .filter( p -> p.supports( runtimeId ) )
                .findFirst();
        if ( runtimeDestroyer.isPresent() ) {
            runtimeDestroyer.get().destroy( runtimeId );
        }
    }
